...
def send_notification(title, content, summary):
    ...
    if not api_key or not uids:
        printf("WxPusher配置不存在，无法推送")
        return
...
def base64Decode(string):
    return base64.b64decode(
        string.translate(str.maketrans("KLMNOPQRSTABCDEFGHIJUVWXYZabcdopqrstuvwxefghijklmnyz0123456789+/", "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"))).decode('utf-8')
...
def get_ep(jduuid: str = ''):
    if not jduuid:
        jduuid = ''.join(str(uuid.uuid4()).split('-'))  # randomstr(16)
...
def getcookie_wskey(key):
    ...
    for num in range(0, 5):
        try:
            ...
            if not sign:
                printf(f"【警告】{unquote(pin)}第{num + 1}次获取sign失败，重试")
                continue
            ...
            resp = post(url=url, headers=headers, data=body, verify=False, proxies={"http": proxys, "https": proxys})
            try:
                token_json = resp.json()
            except Exception:
                printf(f"【错误】{unquote(pin)}第{num + 1}次返回不是JSON：{resp.text}")
                token_json = {}
        ...
        if token != "xxx":
            ...
            printf(f"【警告】{unquote(pin)}第{num + 1}次获取token失败，5秒后重试")
            time.sleep(5)
            randomuserAgent()
            if os.environ.get("DY_PROXY") is not None:
                proxys = get_proxy_api(proxy_url)
    if token == "xxx":
        printf(f"【错误】{unquote(pin)}多次获取token失败，跳过此wskey")
        return "Error"
    for num in range(0, 5):
        ...
        for data in datas:
            if "pt_key" in data['value']:
                try:
                    ...
                    pass
        if reamrk is not None and reamrk != "":
...
def main():
    ...
    if config == "":
        ...
        return
    try:
        if os.environ.get("WP_APP_TOKEN_ONE") is None or os.environ.get("WP_APP_MAIN_UID") is None:
    ...
    if (isinstance(datas, int) and datas > 0) or (isinstance(datas, list) and len(datas) > 0):
        printf("\n===============开始转换JD_WSCK==============")
    else:
    ...
    if envtype in ('ql', ''):
        for data in datas:
            ...
            if cookie and isinstance(cookie, str) and "app_open" in cookie:
